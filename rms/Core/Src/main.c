...
uint16_t findOffset(uint16_t *buffer, uint16_t length)
{
  uint32_t sum = 0;
...
uint16_t calculateRMS(uint16_t *buffer, uint16_t length, uint16_t offset)
{
  int64_t sum = 0;
  for (uint16_t i = 0; i < length; i++)
  {
    int32_t centered = (int32_t)buffer[i] - offset;

    sum += (uint64_t)(centered * centered);
  }
  return (uint16_t)sqrtf((float)sum / length);
}

void ProcessAudioBlock(uint16_t *buffer, uint16_t length)
{
  const uint16_t offset = findOffset(buffer, length);
  ...
  {
    // RMS-Berechnung
    uint16_t blockRMS = calculateRMS(&buffer[i * block_size], block_size, offset);
